...
class StochasticBoostARoota(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, metric=None, selection_algorithm='xgb', model_spec=None, 
                 ...
                 frac=1, seed=1, sampling='full', silent=False):
        ...
        if (seed is None) | (seed == 0):
            # сообщение о том, что нельзя задать значение None или 0 для seed
            raise ValueError("This value must be greater than 0, was " + str(seed))
        if cutoff <= 0:
    ...
    def _create_shadow(self, x_train, seed):
        ...
        np.random.seed(seed ** 2 + x_shadow.shape[1] ** 2)
